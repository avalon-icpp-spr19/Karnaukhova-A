...
int main()
{
	...
	int mas7[100];
	for (int so = 0; so <= 99; ++so)
	{
		mas7[so] = GetRandomValue(1, 100);
	}
	//Вывести на экран
	//Элементы в записи которых встречается цифра 2
	for (int rr = 0; rr <= 99; ++rr)
	{
		if ((mas7[rr] % 2 == 0) || (mas7[rr]/ 10 ==2))
		{
			cout << mas7[rr] << endl;
		}
	}
	//Элементы в записи которых не встречаются нечетные цифры
	for (int dd = 0; dd <= 99; ++dd)
	{
		if ((mas7[dd] % 2 == 0) && ((mas7[dd] / 10) %2== 0))
		{
			cout << mas7[dd] << endl;
		}
	}
